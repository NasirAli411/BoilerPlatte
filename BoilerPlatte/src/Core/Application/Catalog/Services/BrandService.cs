...
namespace BoilerPlatte.Application.Catalog.Services;
...
public class BrandService : IBrandService
{
    ...
    {
        bool brandExists = await _repository.ExistsAsync<Domain.Catalog.Brand>(a => a.Name == request.Name);
        ...
        var brandId = await _repository.CreateAsync<Domain.Catalog.Brand>((Domain.Catalog.Brand)brand);
    ...
    {
        var brandToDelete = await _repository.RemoveByIdAsync<Domain.Catalog.Brand>(id);
    ...
    {
        return await _repository.GetSearchResultsAsync<Domain.Catalog.Brand, BrandDto>(filter.PageNumber, filter.PageSize, filter.OrderBy, filter.AdvancedSearch, filter.Keyword);
    }

    public async Task<Result<Guid>> UpdateBrandAsync(UpdateBrandRequest request, Guid id)
    {
        var brand = await _repository.GetByIdAsync<Domain.Catalog.Brand>(id);
        ...
        await _repository.UpdateAsync<Domain.Catalog.Brand>(updatedBrand);
        await _repository.SaveChangesAsync();
        return await Result<Guid>.SuccessAsync(id);
    }
